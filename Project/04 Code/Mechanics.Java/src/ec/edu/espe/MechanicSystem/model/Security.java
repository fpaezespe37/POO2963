...
import ec.edu.espe.MechanicSystem.view.frmMenuSuperUser;
...
public class Security {
    ...
    public void validLoginAdmin(String users[], String user, String password,int tries){
        boolean found = false;
        ...
        for(int i=0;i<users.length;i++){
            if ((users[i].equalsIgnoreCase(user)&& users[i+1].equals(password))){
                ...
                tries=0;
                login.setTries(tries);
                frmMenuSuperUser frmmenuprincipal = new frmMenuSuperUser();
                frmmenuprincipal.setVisible(true);
                break;
